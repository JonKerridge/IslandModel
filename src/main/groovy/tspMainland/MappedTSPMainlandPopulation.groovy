...
class MappedTSPMainlandPopulation implements MainlandPopulation{
 ...
 */
  ...
  MainlandIndividual convergence(def convergenceLimit) {
    int bestLocation = 0
    boolean found = false
    while (!found && (bestLocation != individuals)){
      if (population[bestLocation].fitness < convergenceLimit)
        found = true
      else
